...
public class CityGenerator : EditorWindow
{
    ...
    private float roadWidh = 5f;                    //도로 폭
    ...
    private void OnGUI()                    //에디터 창 UI 그리기
    {
        ...
        roadWidh = EditorGUILayout.FloatField("Road Width", roadWidh);              //도로 폭 입력
    ...
    private void CreateBuilding(Vector3 position , Transform parent)
    {
        ...
        building.transform.localScale = new Vector3(buildingSpacing - roadWidh -1f, height, buildingSpacing - roadWidh - 1f);
        building.transform.SetParent(parent);                   //Building 그룹 아래로 넣기

        Renderer renderer = building.GetComponent<Renderer>();                          //색 변경을 위한 Renderer
        renderer.material.color = new Color(Random.Range(0.5f, 0.8f), Random.Range(0.5f, 0.8f), Random.Range(0.5f, 0.8f), Random.Range(0.5f, 0.8f));
    ...
    private void CreatRoad(Vector3 position , Transform parent)
    {
    ...
    private void GenerateCity()
    {
        ...
        roadParent.transform.SetParent(cityParent.transform, false);
        ...
        for (int x = 0; x < gridSizeX; x++)
        {
            for(int z = 0; z < gridSizeZ; z++)
            {
                ...
                if (x% 2 == 0 || z % 2 == 0)
                {
                    CreatRoad(position, roadParent.transform);
                }
                else
                {
