...
public class PlayerController : MonoBehaviour
{
    ...
    public float landingDuration = 0.3f;                                //착지 후 착지 모션 지속 시간 (해당 지속 시간 이후에 캐릭터가 움직일 수 있게)
    ...
    void HandleAttack()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Alpha1) && !isAttacking)                   //공격 중이 아닐때 키를 누르면 공격
        {
            ...
            if (animator != null)
            {
                ...
                EffectManager.instance.PlayEffectWithDelay("공격이펙트", effectPos.transform.position, Quaternion.identity, 0.5f, 1.0f);
            }
        }
    }
