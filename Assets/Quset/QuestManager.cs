...
public class QuestManager : MonoBehaviour
{
    ...
    [Header("퀘스트 목록")]
    public QusetData[] availableQuests;                                         //가지고 있는 퀘스트 목록

    private QusetData currentQuest;                                             //진행중인 퀘스트 데이터
    private int curretQuestIndex = 0;                                           //퀘스트 목록중에 진행 중인 번호
    ...
    //퀘스트 시작
    public void StartQuest(QusetData quest)
    {
    ...
    void CheckDeliveryProgress()
    {
        ...
        if (distance <= currentQuest.deliveryRedius)                                                    //유저의 거리가 도착 범위 안쪽인지 검사
        {
    ...
    public void CompleteCurrentQuest()
    {
        ...
        //다음 퀘스트가 있으면 시작
        curretQuestIndex++;
        if (curretQuestIndex < availableQuests.Length)
        {
            StartQuest(availableQuests[curretQuestIndex]);
        }
        else
        {
            currentQuest = null;
